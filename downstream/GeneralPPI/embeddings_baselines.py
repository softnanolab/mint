...
def main(args):
    ...
    if 't5' in args.model_name:
        ...
        model = ProtT5(model_name=args.model_name, 
                       layers='last', 
                       devices=devices, 
                       batch_size=args.bs,
        ...
        model = ProGen(model_name=args.model_name, 
                       layers='last', 
                       devices=devices, 
                       batch_size=args.bs,
                       max_seq_length=args.max_seq_length)
        ...
        model = ESM(model_name=args.model_name, 
                    layers='last', 
                    devices=devices, 
                    batch_size=args.bs,
                    max_seq_length=args.max_seq_length)  
    ...
    if not os.path.isfile(train_emb_file_name):
        if args.task == 'SKEMPI' or args.task == 'BSA':
            ...
            if args.sep_embed:
                train_inputs = model.encode_two(train_dataset.seqs1, train_dataset.seqs2, how='cat')
            else:
