...
class DesautelsCollateFn:
    ...
    def __init__(self, wt_fasta_file, truncation_seq_length=None):
        ...
        self.wt_chain_ids_single = [torch.ones(c.shape, dtype=torch.int32) * i for i, c in enumerate(self.wt_chains_single)]
...
class DesautelsWrapper(nn.Module):
    ...
    def __init__(self, cfg, checkpoint_path, freeze_percent=0.0, use_multimer=True, device='cuda:0', finetune=True):
        super().__init__()
        self.cfg = cfg
...
def main(args):
    ...
    for train_test_split in [0.005, 0.02, 0.2]:
        ...
        train_loader = torch.utils.data.DataLoader(
            train_dataset, batch_size=args.batch_size, collate_fn=DesautelsCollateFn('rcsb_pdb_2G75.fasta'), shuffle=True
        )
        test_loader = torch.utils.data.DataLoader(
            test_dataset, batch_size=args.batch_size*2, collate_fn=DesautelsCollateFn('rcsb_pdb_2G75.fasta'), shuffle=False
        )
        ...
        if args.use_mlp:
            model = DesautelsWrapper(cfg, args.checkpoint_path, args.freeze_percent, args.use_multimer, 
                                     args.device)
            train(model, train_loader, test_loader, cfg, args)
        else:
            model = DesautelsWrapper(cfg, args.checkpoint_path, args.freeze_percent, args.use_multimer, 
                                     args.device, finetune=False)
            train_embeddings, train_targets = get_embeddings(model, train_loader, device=args.device)
            ...
            if args.wandb:
                wandb.log({"pearson": np.mean(pearsons),
                           "mse": np.mean(mses),
