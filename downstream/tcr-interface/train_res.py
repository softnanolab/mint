...
class ResLevelSystem(pl.LightningModule):
    ...
    def evaluate_model(self, data_loader=None, ):
        ...
        for i, batch in enumerate(data_loader):
            loss_this, pred_, dist_, contact_, mask_ = self.minimum_step(batch, self.device_default)
...
class PPICollateFn:
    ...
    def __call__(self, batches):
        ...
        metadata = {'pdb_chains': [item['pdb_chains'] for item in batches], 
                    'cdr3_seqs': cdr3_list, 
                    'epi_seqs': epi_list}
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', type=str, default='./TEIM/train_teim/configs/reslevel_bothnew.yml')
    parser.add_argument('--freeze_percent', type=float, default=0.95)
    parser.add_argument('--full_checkpoint_path', type=str, default='')
    parser.add_argument('--checkpoint_path', type=str, default='/data/cb/scratch/varun/esm-multimer/esm-multimer/checkpoints/650M_nofreeze_filtered_continue/epoch=0-step=180000.ckpt')
    ...
    for i_split, (train_set_this, val_set_this) in enumerate(zip(train_set, val_set)):
        ...
        wandb.init(
            ...
            group="TCR-interface",
            job_type=f'{args.name}_{split}_{i_split}',
            tags=[split]
        )
        ...
        esm_model = FlabWrapper(cfg, args.checkpoint_path, config.model.ae_model, 0, True, 256, 0.2, device_str)

        if args.full_checkpoint_path != '':
            checkpoint = torch.load(args.full_checkpoint_path, map_location=device_str)
            new_checkpoint = OrderedDict((key.replace('model.', '', 1), value) for key, value in checkpoint['state_dict'].items())  
        ...
        model = ResLevelSystem(esm_model, args, train_set_this, val_set_this, device_str, args.only_int)
...
# python train_res.py --full_checkpoint_path "./logs/esm-m-tcr/epoch=5-step=27294.ckpt"