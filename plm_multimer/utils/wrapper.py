...
class ESMWrapper(pl.LightningModule):
    ...
    def forward(self, batch):
        ...
        mask = (~tokens.eq(self.model.cls_idx)) & (~tokens.eq(self.model.eos_idx)) & (~tokens.eq(self.model.padding_idx))
    ...
    def configure_optimizers(self):
        ...
        if self.args.freeze_self_attn:
            ...
            for name, p in self.model.named_parameters():
                if 'multimer_attn' in name: p.requires_grad = True
        ...
        warmup = torch.optim.lr_scheduler.LinearLR(optimizer,
                        start_factor=1e-12, end_factor=1.0, total_iters=self.cfg.warmup_updates)
        decay = torch.optim.lr_scheduler.LinearLR(optimizer,
                        start_factor=1.0, end_factor=self.cfg.end_learning_rate / self.cfg.lr[0], 
                        total_iters=int(0.9 * int(self.cfg.total_num_update)))
        scheduler = torch.optim.lr_scheduler.SequentialLR(optimizer,
                    schedulers=[warmup, decay], milestones=[self.cfg.warmup_updates])
